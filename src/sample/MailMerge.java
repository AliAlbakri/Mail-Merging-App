...
public class MailMerge {
	 ...
	 */
    public static boolean makePDF(ArrayList<String> messageToBeSent, long numberofPages)
    {
    	  try {
    	    //You should change the file name
			  FileChooser savefile = new FileChooser();
			  savefile.getExtensionFilters().add(new FileChooser.ExtensionFilter("PDF doc", "*.pdf"));
			  File selectfile = savefile.showSaveDialog(new Stage());
            OutputStream file = new FileOutputStream(selectfile.getAbsolutePath());
	    ...
	    //keep the following 3 lines unchanged
            ...
            {
                document.add(new Paragraph(messageToBeSent.get(i)));
				//go to new page
				document.newPage();
            }
            document.close();
